<?php 
header('X-XSS-Protection:0'); // ai function ar jono chorom auto XSS protection/prevent debana

?>

<!DOCTYPE html>
<html lang="en">
<head>
    <title>4 html web page</title>
    <!-- Required meta tags -->
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <!-- Style CSS -->
    <!-- Google Fonts -->
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Roboto:300,300italic,700,700italic">

    <!-- CSS Reset https://necolas.github.io/normalize.css/ -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/normalize/8.0.1/normalize.css">

    <!-- Milligram CSS https://milligram.io/ -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/milligram/1.4.1/milligram.css">

    <!-- You should properly set the path from the main file. -->
    <link rel="stylesheet" href="style.css">
    <!-- php.local.com c:\windows\system32\drivers\etc\hosts -->
    
</head>
<body>
    <div class="container">
        <div class="row navbar">
            <div class="column column-60 column-offset-20">
                <ul>
                    <li><a href="/">Home</a></li>
                    <li><a href="from.php">From</a></li>
                    <li><a href="from1.php">From 1</a></li>
                    <li><a href="select.php">Select</a></li>
                    <li><a href="file.php">File</a></li>
                    <li><a href="files.php">Files</a></li>
                </ul>
            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="column column-60 column-offset-20">
                <h2>GET & POST = HTTP VERB</h2>
                <p>Our 1st From $ check XSS = Cross site scripting</p>
            </div>
        </div>
        <div class="row">
            <div class="column column-60 column-offset-20">
                <p>
                    <?php if (isset($_GET['fname']) && !empty($_GET['fname']) ): ?><!-- this is alter native sintext -->
                        First Name: <?php echo $_GET['fname']; ?> <br/>
                    <?php endif; ?>

                    <?php
                    // _GET thaka _POST er priority bashi
                    // _GET & _POST akshatha received korarjon _REQUEST use kora jai

                    if (isset($_REQUEST['lname']) && !empty($_REQUEST['lname']) ){ ?><!-- this is normal sintext -->
                        Last Name: <?php echo $_REQUEST['lname']; ?> <br/>
                    <?php } ?>
                    
                        
                    
                </p>
            </div>
        </div>
        <div class="row">
            <div class="column column-60 column-offset-20">
                <form action="">
                    <label for="fname">First Name</label>
                    <input type="text" name="fname" id="fname">

                    <label for="lname">Last Name</label>
                    <input type="text" name="lname" id="lname">

                    <button type="submit">Submit</button>

                </form>
            </div>
        </div>
    </div>
    <!-- cd 4-html-web-page -->
    <!-- php -S localhost:3030 -->
    <!-- php -S php.local.com:3030 -->


    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->

</body>
</html>
